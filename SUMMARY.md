...
A pure-R package for creating sunburst, icicle, donut, and tree adjacency
diagrams using ggplot2. Reimplements the Python-dependent `ggsunburst`
package entirely in R.
...
- **Parsers**: Newick (via `ape::read.tree()`), lineage files, node-parent
  CSV/TSV, parent-child data.frames
...
- **Label engine**: radial angles, flip logic for readability, delta_angle,
  xfraction
- **S3 class**: `sunburst_data` with `print()`, `plot()`,
  `as.data.frame()`, `$data` alias
- **Plot functions**: `sunburst()` (coord_polar), `icicle()`
  (scale_y_reverse)
...
- **`ape::phylo` input**: direct phylo objects, no Newick conversion needed
- **Path-delimited strings**: `c("A/B/C", "A/B/D")` with configurable
  separator
- **Path-column data.frames**: `data.frame(path = ...)` with extra columns
- **`data.tree::Node` input**: R6-based trees with custom scalar fields
- **`highlight_nodes()`**: add a highlight layer by node name or ID
...
- **`donut()`**: ring chart (sunburst restricted to N outermost levels)
- **`ggtree()`**: dendrogram with horizontal, vertical, and circular
  layouts plus scale bars
- **`drilldown()`**: extract subtree and recompute as new root
- **`bars()`**: bar chart annotation layer for leaf nodes
- **`tile()`**: heatmap-style tile annotation layer
...
- **Label filtering**: `min_label_angle` to hide labels on narrow sectors
...
- **`geom_sunburst()`**: custom StatSunburst ggproto for
  `ggplot(df) + geom_sunburst(aes(id, parent))`
- **`theme_sunburst()`**: tailored theme with centred title, bottom legend
...
### Data preparation
- `sunburst_data()` — main entry point, all input types
- `drilldown()` — subtree extraction and recomputation

### Standalone plot constructors
- `sunburst()` — polar sunburst
- `icicle()` — rectangular icicle
- `donut()` — ring chart
- `ggtree()` — dendrogram

### ggplot2 geom
- `geom_sunburst()` — idiomatic ggplot2 layer

### Annotation layers (added to existing plots)
- `highlight_nodes()` — emphasis layer
- `bars()` — bar chart annotation
- `tile()` — heatmap annotation
- `sunburst_multifill()` — per-depth fill (sunburst)
- `icicle_multifill()` — per-depth fill (icicle)

### Utilities
- `nw_print()` — tree inspection
- `theme_sunburst()` — custom theme

### S3 methods
- `print.sunburst_data()`, `plot.sunburst_data()`,
  `as.data.frame.sunburst_data()`, `$.sunburst_data`
...
The internal tree is a plain list with integer-indexed node IDs (`nodes`,
`children`, `parent`, `root`, `n_tips`). It is never exposed to users.

## Key Design Decisions

1. **`ape::read.tree()` for Newick** — leverages a well-tested CRAN
   package rather than writing a custom parser
2. **List-based tree** — simple, efficient for traversal, not exported
3. **Pre-computed coordinates + `geom_rect()`** — debuggable, composable
   with ggplot2; custom ggproto added later in v0.5
...
- **TDD throughout**: every function had failing tests before
  implementation
- **OpenSpec change management**: 33 changes with proposal, design, specs,
  and tasks
