...
Multiple functions use `rlang::abort("Column '{fill}' not found...")` where
the `{fill}` is a literal string, not interpolated. Switching to
`cli::cli_abort()` across the package would give proper interpolation.

**Affected functions:** `sunburst()`, `icicle()`, `donut()`, `drilldown()`,
`bars()`, `tile()`, `.validate_fill()`

**Sources:** S1 in COMMENTS-plot-donut, S2 in COMMENTS-drilldown,
S1 in COMMENTS-025-fill-auto-depth
...
would catch visual regressions (wrong colour, broken polar mapping, label
positioning) that structural tests miss.
...
**Sources:** S1 in COMMENTS-012-plot-sunburst, S1 in COMMENTS-plot-ggtree,
S2 in COMMENTS-plot-bars, S1 in COMMENTS-plot-tile
...
`parse_newick()` and `nw_print()` both have the same
`tryCatch(withCallingHandlers(...))` pattern for reading Newick with
warning capture. A shared `.read_newick_safe()` returning a `phylo`
object would deduplicate.

**Source:** S1 in COMMENTS-nw-print

### Fill/validation logic in plot functions
`sunburst()`, `icicle()`, and `donut()` share identical input validation
and fill branching. This was partially addressed in v0.5 with shared
helpers (`.resolve_fill()`, `.validate_fill()`, `.build_rect_layer()`).
Review whether further extraction is needed.
...
`compute_label_positions()` calls `get_leaves(tree, nid)` for every node.
Pre-computing a `leaf_size_sum` map in a single postorder pass would be
...
computing angular fractions. Either validate in `assign_sizes()` or handle
in `compute_coordinates()`.

**Source:** S1 in COMMENTS-007-compute-sizes

### Single-leaf tree for bars/tile
No tests for `bars()` or `tile()` on a single-leaf tree.
...
`data.tree::Node` objects are silently excluded.
...
### Tibble input
No test confirms `sunburst_data()` works with tibble input (as opposed to
plain data.frame). Should work via inheritance but worth documenting.

**Sources:** S1 in COMMENTS-005-parse-dataframe, S1 in COMMENTS-006-detect-input
...
SPEC.md mentions `fill = NULL` may change to mean `"auto"` in v0.6+.
The roxygen2 docs don't mention this. A note would help users write
forward-compatible code.

**Source:** S3 in COMMENTS-025-fill-auto-depth

### Bare name fill example in roxygen2
The examples show `fill = "depth"` but not the bare name form `fill =
depth`.
...
These exist for API consistency (all justification functions take `angle`).
Could be constants instead. Current approach is fine.
...
node is duplicated. Faithful to original behaviour but may surprise users.

**Source:** S1 in COMMENTS-004-parse-file-inputs

### `parse_node_parent()` error uses cli glue in `abort()`
Works due to rlang-cli integration, but `cli::cli_abort()` would be
clearer.
